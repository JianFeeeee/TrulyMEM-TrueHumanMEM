...
步骤2: task_query (查询工作记忆链/最近任务) → 必须第二步执行。**使用 task_query 替代 memory_recall 查询任务**
步骤3: 处理对话内容
步骤4: memory_commit (写入关键信息) → 将用户明确提到的重要信息写入图数据库
步骤5: 更新工作记忆链
步骤6: task_archive (归档已完成/过期的旧任务) → **必须最后执行**
```

**违反顺序的后果**：
- 跳过步骤1 → 无法获取人设，回复风格错误
- 跳过步骤2 (task_query) → 无法获取任务上下文，对话不连贯（不要用 memory_recall 替代查询任务）
- 跳过步骤6 → 旧任务从不归档，工作记忆无限膨胀
- 顺序错误 → 系统状态混乱
